...
class DashboardGenerator:
    """Class to generate and render dashboard components"""
    
    def __init__(self, df):
        """Initialize with dataframe"""
        self.df = df
    
    def render_chart(self, filtered_df, chart_config):
        ...
        try:
            if chart_type == "bar":
                ...
                # Ensure columns exist in the dataframe
                if value_col not in filtered_df.columns or name_col not in filtered_df.columns:
    ...
    def render_filter(self, filter_config, key_suffix=""):
        ...
        try:
            ...
            if filter_type == "range":
                ...
                options = self.df[column].dropna().unique().tolist()
                st.multiselect(
                    label,
                    options=options,
                    default=options,
    ...
    def apply_filters(self, filter_configs):
        ...
        for filter_config in filter_configs:
            try:
                ...
                key = f"filter_{column}_dash_{filter_configs.index(filter_config)}"
                
                if key not in st.session_state:
                    continue
                
                filter_value = st.session_state[key]
        ...
        try:
                ...
                if col_type == "Numeric":
                    ...
                    if analysis_depth == "Comprehensive":
                        try:
                            ...
                            report += f"* *Error calculating additional statistics: {str(e)}*\n"
                    ...
                    try:
                        ...
                        report += f"* *Error analyzing categorical values: {str(e)}*\n\n"
                        ...
                        if analysis_depth in ["Standard", "Comprehensive"]:
                            ...
                            if analysis_depth == "Comprehensive":
                                ...
                                try:
                                    ...
                                    report += f"* *Error analyzing day of week: {str(e)}*\n"
            ...
            if len(analyzed_num_cols) >= 2:
                ...
                try:
                    ...
                    report += f"*Error in correlation analysis: {str(e)}*\n\n"
            ...
            try:
                ...
                report += f"*Error in missing value analysis: {str(e)}*\n\n"
                ...
                try:
                    ...
                    for col in analyzed_num_cols:
                        try:
                            ...
                            report += f"*Error analyzing outliers for column {col}: {str(e)}*\n\n"
                    ...
                    if len(outlier_df) > 0:
                        ...
                        report += "*Error analyzing outliers or no outliers found.*\n\n"
                except Exception as e:
                    report += f"*Error in outlier analysis: {str(e)}*\n\n"
            ...
            if missing_values > 0:
                ...
                # Top columns with missing values
                if 'cols_with_missing' in locals() and len(cols_with_missing) > 0:
                    top_missing = cols_with_missing.head(3)
                    if len(top_missing) > 0:
                        missing_cols = ", ".join([f"'{col}' ({pct}%)" for col, pct in zip(top_missing['Column'], top_missing['Missing %'])])
                        report += f"  * Columns with most missing values: {missing_cols}\n"
            ...
            # Outliers (if analyzed)
            if len(analyzed_num_cols) > 0 and 'outlier_df' in locals() and len(outlier_df) > 0:
                outlier_cols = outlier_df[outlier_df['Outlier %'] > 5]
                if len(outlier_cols) > 0:
                    report += f"* **Outliers:** {len(outlier_cols)} columns have more than 5% outliers.\n"
            ...
            # Outlier recommendations
            if len(analyzed_num_cols) > 0 and 'outlier_df' in locals() and len(outlier_df[outlier_df['Outlier Count'] > 0]) > 0:
                report += "* **Outlier Treatment:**\n"
                report += "  * Inspect outliers to determine if they are valid data points or errors.\n"
                report += "  * Consider appropriate treatment such as capping, transforming, or removing outliers depending on your analysis goals.\n"
            ...
            for col in analyzed_cat_cols[:3]:  # Limit to first 3 columns
                try:
                    ...
                    plot_df = filtered_df[col].value_counts().reset_index()
                    plot_df.columns = [col, 'Count']  # Rename columns
                    ...
                    st.error(f"Error creating bar chart for {col}: {str(e)}")
            ...
            with col2:
                ...
                try:
                    ...
                    st.warning(f"HTML export requires the markdown package: {str(e)}. Try downloading as Markdown instead.")
...
def render_trend_analysis():
    ...
    if analysis_type == "Time Series Plot":
        ...
        try:
            ...
            fig.add_trace(
                go.Bar(
                    x=period_stats.index.astype(str),  # Convert to string to ensure compatibility
                    y=period_stats['mean'],
                    name="Mean",
                    error_y=dict(
                        type='data',
            ...
            if period_type in ["Month", "Day of Week"]:
                ...
                if period_type == "Month":
                    ...
                    # Reorder months
                    if set(period_order).issubset(set(heatmap_data.columns)):
                        heatmap_data = heatmap_data[period_order]
...
def render_distribution_analysis():
    ...
    if analysis_type == "Histogram":
        ...
        try:
            ...
            st.error(f"Error in Q-Q plot analysis: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                st.error(f"Error in fitting distributions: {str(e)}")
                st.exception(e)
                return
